...
export async function generateText(
  ...
  const prompt = `Give me an itinerary for this days: ${dayStart} ${dayEnd} in ${place} with this title ${title} ,with this JSON schema,for the activities place, take into account the description (${description}) and it can be different from the Itinerary place, also make it an object like this one ${Itinerary}: 
    { "type":"object", "properties": { "title": {"type":"string"}, "description": {"type":"string"}, "dayStart":{"type":"date"},"dayEnd":{"type":"date"}, "activities": {"type":"array", "items": { "type":"object", "properties": { "name": {"type":"string"}, "description": {"type":"string"} "transport": {"type":"boolean"},"outdoor": {"type":"boolean"},"scheduleStart":{"type":"string"},"scheduleEnd":{"type":"string"}"place": 
    ...
    If there is not a place for the activity,use the place ${place} for the activity.
