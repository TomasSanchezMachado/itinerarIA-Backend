...
export async function addWithAI(req: Request, res: Response) {
  try {
    ...
    const ages = participants.map((participant: Participant) => participant.age);
    const preferences = participants.reduce((acc: any[], participant: Participant) => {
      if (participant.preferences) {
        return acc.concat(participant.preferences.map((preference: any) => preference.name));
      }
      return acc;
    }, []) as Preference[];
    ...
    participants.forEach(async (participant: Participant) => {
      if (participant.id && participant.id === undefined) {
        // Creo el participante si no existe
        const participantCreated = em.create(Participant, {...participant, preferences: participant.preferences.map((preference:Preference) => new ObjectId(preference.id))});
        console.log(participantCreated, 'participantCreated');
        itinerary.participants.add(participantCreated);
      } else {
        // Si ya existe (de favoritos), lo busco y lo agrego
